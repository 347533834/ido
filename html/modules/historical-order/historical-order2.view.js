define(function (require, exports, modules) {
    ...
    var template = require('text!./historical-order2.tpl.html'),
        util = require('util');
    ...
    var View = Backbone.View.extend({
        ...
        events: {
            'click .historical-order .more': function () {
                this.load_afternoon_history_order(this.page);
            },
        },
        initialize: function () {
        ...
        load_afternoon_history_order: function (p) {
            var _this = this;
            var page = p ? p : 1;
            window.jsonpGet('Trade/load_afternoon_history_order', {
                token: this.user.token,
                page: page,
            }, 'load_afternoon_history_order', function (json) {
                ...
                if (json.code == 1) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        str += '<li>\n' +
                            '           <div class="scene">\n' +
                            '               <p>\n' +
                            '                   <span>' + (data[i].type == 1 ? '购买' : '出售') + '(' + data[i].name + ')</span>\n' +
                            '                   <span style="background-color: ' + (data[i].status == 2 ? 'green' : '') + '">' + (data[i].status == 2 ? '已成交' : '已撤销') + '</span>\n' +
                            '               </p>\n' +
