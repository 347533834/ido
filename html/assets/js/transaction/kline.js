...
function Fill(data, summary, sysdatetime, isOpen) {
    //从默认开始时间到当前服务器时间段的数据
    var result = [];
...
var highStockChart = function (divid, data, intFormatFloat) {
    if (!intFormatFloat) intFormatFloat = 4;
    ...
    return new Highcharts.StockChart({
        ...
        tooltip: {
            formatter: function () {
                ...
                open = parseFloat(this.points[0].point.open.toFixed(intFormatFloat));
                high = parseFloat(this.points[0].point.high.toFixed(intFormatFloat));
                low = parseFloat(this.points[0].point.low.toFixed(intFormatFloat));
                close = parseFloat(this.points[0].point.close.toFixed(intFormatFloat));
                y = this.points[1].point.y;
                var tip = '<b>' + Highcharts.dateFormat('%Y-%m-%d %A', this.x) + '</b><br/>';
                tip += '开盘价：' + open.toFixed(4) + '<br/>';
                tip += '收盘价：' + close.toFixed(4) + '<br/>';
                tip += '最高价：' + high.toFixed(4) + '<br/>';
                tip += '最低价：' + low.toFixed(4) + '<br/>';
                if (open < close) {
                    tip += '涨跌额：<span style="color:#F56363;">+' + parseFloat(zde).toFixed(4) + '</span><br/>';
                    tip += '涨跌幅：<span style="color:#F56363;">+' + (zdf * 100).toFixed(2) + '%</span><br/>';
                } else if (open > close) {
                    tip += '涨跌额：<span style="color:#5B910B;">' + parseFloat(zde).toFixed(4) + '</span><br/>';
                    tip += '涨跌幅：<span style="color:#5B910B;">' + (zdf * 100).toFixed(2) + '%</span><br/>';
                } else {
                    tip += '涨跌额：<span>' + parseFloat(zde).toFixed(4) + '</span><br/>';
