...
function nameMask($name){
    $lengs = strlen($name);
    if ($lengs > 6 && $lengs <= 9){
        return $name?mb_substr($name, 0,-2).'**':'';
    }elseif ($lengs > 9){
        return $name?mb_substr($name, 0,-3).'**':'';
    }else{
 ...
 * @return int
 */
function is_realname($s)
{
    return preg_match("/^[\x{4e00}-\x{9fa5}0-9]{2,10}$/u", $s) || preg_match("/^[a-zA-Z0-9]{4,20}$/u", $s);
}
...
{
    return preg_match("/^[\x{4e00}-\x{9fa5}0-9]{2,5}$/u", $s) || preg_match("/^[a-zA-Z0-9]{2,8}$/u", $s);
}
...
{
    return preg_match('/^1[123456789][0-9]{9}$/', $s);
}
 ...
 * @return bool
 */
function success_validate($geetest_validate){
    if(!$geetest_validate && session('geetest_validate') === $geetest_validate){
        session('geetest_validate',null);
        return true;
    }
    return false;
}
...
function gt_data($user_id){
    //todo 预留系统拆分时间
    $action_arr = ['Login/app','Login/loading','User/refresh'];
    $ids = [-10001,-10445,10028];
    if(in_array($user_id,$ids)) return true;
    if(time()>=strtotime(date('Y-m-d').' 01:00:00')&&time()<=strtotime(date('Y-m-d').' 00:00:00')&&!in_array(request()->controller().'/'.request()->action(),$action_arr)&&(!in_array($user_id,$ids))&&$user_id!=''){
        return false;
    }
    return true;
}
 ...
 * @return int|string
 */
function get_rand($proArr = array(90, 10)) {
    $result = '';
    // 概率数组的总概率精度
...
function get_coin_price($coin_name)
{
    // 从火币获取价格
    $client = new \Huobi('53db6fad-f3ba2380-09dfedb5-11aae', '48458c72-b9945bcc-40a724b5-aeacc');
    switch ($coin_name) {
        case 'ETH':
            // 从火币获取价格
            $result = $client->get_history_kline('ethusdt', '1min', 1);
            if (!$result){
                $price = null;
            }else{
                $coin_data = $result['data'][0];
                if (Db::name('kline_eth_usdt_min_huobi')->where(['id' => $coin_data['id']])->count())
                    Db::name('kline_eth_usdt_min_huobi')->update($coin_data);
                else Db::name('kline_eth_usdt_min_huobi')->insert($coin_data);
        ...
        case 'SHT':
            ...
            break;
        case 'BTC':
            $result = $client->get_history_kline('btcusdt', '1min', 1);
            if (!$result){
                $price = null;
            }else{
                $coin_data = $result['data'][0];
...
function gt_sht_price(){
//    return 0.5;
    $list = http('https://www.shtcoin.top/klinevtwo/indexv1');
    $price = 1;
    foreach ($list[0]['data'] as $item){
        if($item['name']=='SHT_USDT') {
            $price = $item['currentExchangPrice'];
            $item['id'] = time();
            if (Db::name('kline_sht_usdt_min_shtcoin')->where(['id' => $item['id']])->count())
                Db::name('kline_sht_usdt_min_shtcoin')->update($item);
            else Db::name('kline_sht_usdt_min_shtcoin')->insert($item);
        }
    }
    return $price;
}